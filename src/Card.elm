...
view attrs { card, state } =
    Html.div
        ([ Attr.class (toString card ++ " h-32 w-24 transform-gpu perspective-600")
         ]
            ++ attrs
        )
        [ Html.div
            [ Attr.class "card w-full h-full relative"
            , Attr.style "transition" "transform 1s"
            , Attr.style "transform-style" "preserve-3d"
            , Attr.classList [ ( "rotate-y-180", state == FaceUp ) ]
            ]
            [ Html.img
                [ Attr.class "w-full h-full absolute"
                , Attr.style "backface-visibility" "hidden"
                , Attr.src "/cards/1B.svg"
                ]
                []
            , Html.img
                [ Attr.class "w-full h-full absolute rotate-y-180"
                , Attr.style "backface-visibility" "hidden"
